...
// HashTable에 친구의 정보를 기억시켜보자.
	public Test02() {
		...
		// 키값만 추출
		Enumeration en = table.keys(); // *** keys() 중요
...
//			System.out.println(en.nextElement());
		...
		// 키값만 추출 2
		Set set = table.keySet(); // *** keySet() 중요
		...
		Set entry = table.entrySet(); // 데이터 출력은 많이 쓰이진 않음
		Iterator it3 = entry.iterator();
		while(it3.hasNext()) {
			Map.Entry en1 = (Map.Entry) it3.next(); // Map.Entry 는 VO 클래스 키값과 데이터가 모두 저장
