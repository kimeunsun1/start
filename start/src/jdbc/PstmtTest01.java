	...
	public PstmtTest01() {
		...
		try {
			...
			*/
			...
			/*
			 	줄완전한 질의명령(? 있는 질의명령)에 데이터를 채우는 방법
			 		...
			 		이때 위치값은 문자열 중에서 ? 의 순서를 의미하고
			 		시작값은 1부터 시작한다.
			 */
			pstmt.setInt(1, dno);
			
			// 질의명령이 완성이 됐으므로 JDBC에게 전달하고 결과를 받는다.
			rs = pstmt.executeQuery();
			// 몇사람이 조회될지 알 수 없으므로
			while(rs.next()) {
				// re.next() ==> 작업실행줄을 한줄 내려주는 함수. 
				// 이때 내린 결과 꺼내올 데이터가 있으면 true 를 반환해준다. 없으면 false...
